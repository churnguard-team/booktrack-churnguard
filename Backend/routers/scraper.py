...
Google Books est beaucoup plus stable (evite les erreurs 503)
et offre des resultats de recherche plus riches et rapides.
...
class ScrapedBook(BaseModel):
    """Modele de donnees pour un livre trouve sur le web"""
    title: str
...
async def scrape_books(
    ...
    """
    Recherche des livres avec un systeme de secours (Fallback).
    1. Essaie Open Library en premier (gratuit, pas de quota IP strict).
    2. Si Open Library echoue (ex: 503), essaie Google Books en secours.
    3. Si les deux echouent (ex: quota 429 Google), retourne une liste vide.
    """
    ...
    try:
        ...
        print(f"[Scraper] Open Library a echoue: {exc}")
    ...
    try:
        ...
        if gb_res.status_code == 200:
            ...
            print(f"[Scraper] Google Books a echoue avec le code {gb_res.status_code}: {gb_res.text[:100]}")

    except Exception as exc:
        print(f"[Scraper] Google Books a rencontre une erreur: {exc}")
