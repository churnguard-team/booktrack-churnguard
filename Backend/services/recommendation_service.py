...
Signaux utilisés :
  - genres_preferes déclarés à l'onboarding
  - genres des livres lus/favoris (user_books)
  - notes données aux livres (user_books.note)
  - nombre de commentaires par livre (book_comments) → popularité sociale
  - churn score XGBoost existant (lecture seule) → boost priorité
  - popularité globale (nb d'ajouts en bibliothèque)
"""
...
def _get_churn_score(db: Session, user_id: str) -> float:
    """Lecture seule du dernier score XGBoost — jamais modifié."""
    row = db.execute(text("""
...
def _get_user_genres(db: Session, user_id: str) -> List[str]:
    """
    Genres préférés = union de :
    1. genres déclarés à l'onboarding (genres_preferes)
    2. genres des livres lus, en cours ou favoris
    3. genres des livres bien notés (note >= 4)
    """
    row = db.execute(text(
...
def recommend_for_user(db: Session, user_id: str, n: int = 10) -> List[Dict[str, Any]]:
    """
    Recommandations personnalisées multi-signaux.

    Score final d'un livre =
        popularité (nb ajouts en bibliothèque)
      + note_moyenne * 2          (qualité perçue)
      + nb_commentaires * 0.5     (engagement social)
      + churn_boost               (si utilisateur à risque > 0.6)
    """
    ...
    # ── Étape 1 : livres des genres préférés ──────────────────────
    if genres:
    ...
    # ── Étape 2 : fallback livres populaires ──────────────────────
...
def get_recommendation_stats(db: Session) -> Dict[str, Any]:
    """Statistiques complètes pour le dashboard modérateur."""
    ...
    # Utilisateurs à risque avec leurs genres préférés
    ...
    # Stats churn distribution depuis la BD
