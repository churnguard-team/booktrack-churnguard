"""
Endpoints dedies a n8n workflow.
Appeles par les outils du chatbot et l'automatisation de retention.
"""
...
def post_similar_books(req: SimilarBooksRequest, db: Session = Depends(get_db)):
    """Retourne des livres similaires."""
    try:
...
def post_explain_recommendation(req: ExplainRequest, db: Session = Depends(get_db)):
    """Explique pourquoi un livre est recommande a un utilisateur."""
    try:
        ...
        if book[2] in genres_user:
            reasons.append(f"correspond a votre genre prefere : {book[2]}")
        if book[5] and float(book[5]) >= 4:
            reasons.append(f"tres bien note ({book[5]}/5)")
        if int(book[6]) > 0:
            reasons.append(f"{book[6]} commentaire(s) de lecteurs")
        if int(book[4]) > 1:
            reasons.append(f"ajoute par {book[4]} lecteurs")
        if not reasons:
        ...
        return {
            "explanation": {
                ...
                "genre": book[2],
                "reasons": reasons,
                "summary": f"'{book[0]}' vous est recommande car il " + ", ".join(reasons) + ".",
                "stats": {
...
def post_retention_trigger(req: RetentionRequest, db: Session = Depends(get_db)):
    """Cree une action de retention (discount) dans la base."""
    try:
...
def post_churn_data(req: ChurnDataRequest, db: Session = Depends(get_db)):
    """Retourne les utilisateurs au-dessus du seuil de risque."""
    try:
