...
Gère la détection des utilisateurs à risque churn et l'envoi de campagnes discount.
Ne modifie pas les modèles ML existants.
"""
...
def get_high_churn_users(db: Session, threshold: float = 0.6) -> List[Dict[str, Any]]:
    """Retourne les utilisateurs dont le churn score dépasse le seuil."""
...
def trigger_retention_campaign(
    ...
    """
    Enregistre une action de rétention (discount) pour un utilisateur à risque.
    Retourne le statut de l'action créée.
    """
    # Vérifie que l'utilisateur existe et a un score churn
    row = db.execute(text("""
    ...
    if not row:
        return {"status": "error", "detail": "Utilisateur non trouvé ou sans score churn"}
    ...
    """), {
        ...
        "uid": user_id,
        "contenu": contenu,
        "sujet": f"Une offre spéciale pour vous, {prenom} !",
    })
    db.commit()
...
def get_similar_books(db: Session, book_id: str, n: int = 5) -> List[Dict[str, Any]]:
    """
    Livres similaires basés sur le même genre + auteur.
    """
    # Récupère le genre et l'auteur du livre source
